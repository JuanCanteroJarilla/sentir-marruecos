import { Metadata } from "next";

export const metadata: Metadata = {
  title: "Viajar con niños a Marruecos | Viajes en familia | Sentir Marruecos",
  description:
    "Organizamos viajes a Marruecos para familias con niños: itinerarios flexibles, actividades educativas, seguridad y diversión para grandes y pequeños. Descubre Marruecos en familia con Sentir Marruecos.",
  keywords: [
    "viajar con niños Marruecos",
    "viajes en familia Marruecos",
    "actividades para niños Marruecos",
    "aventura en familia Marruecos",
    "turismo familiar Marruecos",
    "experiencias con niños Marruecos"
  ],
  openGraph: {
    title: "Viajar con niños a Marruecos | Viajes en familia | Sentir Marruecos",
    description:
      "Organizamos viajes a Marruecos para familias con niños: itinerarios flexibles, actividades educativas, seguridad y diversión para grandes y pequeños.",
    url: "https://sentirmarruecos.es/Especial/viajar-con-ninos",
    siteName: "Sentir Marruecos",
    images: [
      {
        url: "/images/og-image.jpg",
        width: 1200,
        height: 630,
        alt: "Viajar con niños a Marruecos - Sentir Marruecos",
      },
    ],
    locale: "es_ES",
    type: "website",
  },